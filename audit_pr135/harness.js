...
src+='\n;globalThis.__t={'+
  ...
  /* PR15.5-E: arsenal ranged — muzzle/projétil/trail/impacto por família */
  'RANGED_VISUAL_FAMILIES,WEAPON_RANGED_VISUAL_PROFILES,WEAPON_RANGED_VISUAL_FALLBACK,RANGED_PROFILE_BY_DEF,rangedVisualProfile,RV_BODY,RV_TRAIL,RV_MUZZLE,RV_IMPACT,MUZZLE_FX_MAX,IMPACT_FX_MAX,RANGED_FX_POOL_MAX,RANGED_MUZZLE_LIFE_MAX,RANGED_IMPACT_LIFE_MAX,rangedFxSeq:()=>_rangedFxSeq,muzzleVisualPush,impactVisualPush,rangedFxTick,rangedFxClear,drawMuzzleFx,drawImpactFx,drawRangedProjectileBody,drawProjectile,onProjectileHit,explodeOrb,detonateSpecial,getMuzzleFx:()=>muzzleFx,setMuzzleFx:a=>{muzzleFx.length=0;for(let i=0;i<a.length;i++)muzzleFx.push(a[i]);},getImpactFx:()=>impactFx,setImpactFx:a=>{impactFx.length=0;for(let i=0;i<a.length;i++)impactFx.push(a[i]);},'+
