...
ok('npm test continua listando as suítes (18 legadas + PR13 + PR13.5 B2 + PR13.5 B3 + B3-FIX + B4 + B5-A + B5-B + B5-B-FIX + B5.5 + B6 + PR14 B2 + PR14 B3 + PR14 B3-FIX + PR14 B3-FIX.1 + PR14 B4 + PR14 B5 + PR14 B5-FIX.1 + PR14 B5-FIX.2 + DEV-FIX DevTools)',()=>{
  const pkg=JSON.parse(fs.readFileSync(path.join(ROOT,'package.json'),'utf8'));
  const partes=pkg.scripts.test.split('&&').map(s=>s.trim());
  assert.strictEqual(partes.length,36,'36 suítes no npm test');   // PR14 B5-FIX.2 somou a sua
