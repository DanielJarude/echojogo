...
ok('npm test continua listando as suítes (18 legadas + PR13 + PR13.5 B2 + PR13.5 B3 + B3-FIX + B4 + B5-A + B5-B + B5-B-FIX)',()=>{
  const pkg=JSON.parse(fs.readFileSync(path.join(ROOT,'package.json'),'utf8'));
  const partes=pkg.scripts.test.split('&&').map(s=>s.trim());
  assert.strictEqual(partes.length,25,'25 suítes no npm test');
});
